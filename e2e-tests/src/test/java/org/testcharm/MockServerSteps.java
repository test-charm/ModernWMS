...
public class MockServerSteps {
    ...
    @Autowired
    private JData jData;
    @Value("${mock-server.enabled:false}")
    ...
    public void mock_api(String mock) {
        if (!mockServerEnabled) {
            throw new IllegalStateException("mock-server.enabled=false, cannot use Mock API steps");
        }
    ...
    public void 验证mockAPI(String dalExpression) {
        if (!mockServerEnabled) {
            throw new IllegalStateException("mock-server.enabled=false, cannot verify Mock API");
        }
