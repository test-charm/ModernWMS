...
| `is_valid` | 类别状态 | `true` / `false` | 默认回归覆盖“修改自身”和“同步直接子类别”；带孙类别的期望行为单独记录为已知缺陷 |
