...
+ 下载源码后编译
  ...
  + 第四步，初始化数据库
   ...
   1) 修改后端目录`/ModernWMS/backend/appsettings.json`文件，参考 <a href="https://modernwms.ikeyly.com/problem-contents.html?fileurl=/assets/markdown/problem-usingPgsql_zh.md">修改配置文件操作流程</a> ，连接池配置时注意修改数据库IP地址、端口、账号、密码，确保可以正确连接数据库
